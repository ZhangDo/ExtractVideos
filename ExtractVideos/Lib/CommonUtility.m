...
+ (BOOL)isValidMobilePhoneNumber:(NSString*)number
{
    ...
    dispatch_once(&onceToken, ^{
        checker = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", @"^1\\d{10}$"];
    });
...
+(NSString *)formatterWithNumber:(long long)value andSize:(NSUInteger)size andGroupingSeparator:(NSString *)groupingSeparator
{
    NSNumber *number = [NSNumber numberWithLongLong:value];
    NSNumberFormatter *formatter = [NSNumberFormatter new];
    [formatter setUsesGroupingSeparator:YES];
    [formatter setGroupingSize:size];
    [formatter setGroupingSeparator:groupingSeparator];
    return [formatter stringFromNumber:number];
}

#pragma mark --- 生成水印图片
+(UIImage *)addImage:(UIImage *)backgroundImage maskImage:(UIImage *)watermarkImage
{
    UIGraphicsBeginImageContextWithOptions(backgroundImage.size, NO, 1.0);
    [backgroundImage drawInRect:CGRectMake(0, 0, backgroundImage.size.width, backgroundImage.size.height)];
    [watermarkImage drawInRect:CGRectMake((backgroundImage.size.width - watermarkImage.size.width)/2.0,
                                          (backgroundImage.size.height - watermarkImage.size.height)/2.0,
